      ...
      <form action="#">
        <div class="flex flex-col bg-white p-10 rounded-lg shadow space-y-6">
          ...
          <div class="flex-col-reverse sm:flex-row sm:justify-between items-center">
            <button @click="login" class="bg-blue-500 text-white font-bold px-5 py-2 rounded focus:outline-none shadow hover:bg-blue-700 transition-colors">Log In</button>
          </div>
...
methods:{
  login(){
    if(this.username == '' || this.password == ''){
      ...
      const adminID = 'adminauthorized';
      this.$router.push(`/adminhomepage/${adminID}`);
    } else {
      alert('use not authorized');
    }
  },
},
